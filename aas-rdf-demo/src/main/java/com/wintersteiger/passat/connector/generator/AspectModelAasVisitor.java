...
import org.eclipse.esmf.aspectmodel.aas.LangStringMapper;
import org.eclipse.esmf.aspectmodel.aas.PropertyMapper;
...
public class AspectModelAasVisitor implements AspectVisitor<Environment, Context> {
   ...
   private Optional<SubmodelElement> mapText( final Property property, final Context context ) {
      ...
      if ( !property.getPayloadName().isEmpty()
    		  // do not exclude SubmodelElementCollection & ..List
...
//            && !( element instanceof SubmodelElementCollection )
   ...
   private SubmodelElementCollection mapToAasSubModelElementCollection( final Entity entity, final Property property,
      ...
      return new DefaultSubmodelElementCollection.Builder()
            ...
            // add semanticId to the corresponding ConceptDescription
    ...
    */
   private Reference buildReferenceToConceptDescription( final org.eclipse.esmf.metamodel.Property property ) {
      final Key key = new DefaultKey.Builder()
            .type( KeyTypes.CONCEPT_DESCRIPTION )
            .value( DEFAULT_MAPPER.determineIdentifierFor( property ) )
            .build();
   ...
   /**
    * build a reference to the complex type of an entity characteristic
    */
	private List<Reference> buildIsCaseOfReference(Optional<Characteristic> characteristic) {
		// whenever 
		if (characteristic.isPresent()) {
			Optional<Type> dataType = characteristic.get().getDataType();
			if ( dataType.isPresent()) {
				if ( dataType.get().isComplexType() ) {
					return List.of( new DefaultReference.Builder()
							.type(ReferenceTypes.EXTERNAL_REFERENCE)
							.keys(new DefaultKey.Builder()
									.type(KeyTypes.GLOBAL_REFERENCE)
									.value(dataType.get().getUrn())
									.build())
							.build());
				}
			}
